...
```mermaid
graph TD
    subgraph DriverHomeViewport [Driver Idle Dashboard Viewport]
        TopStatusPill["Online/Offline Status Switch ('GO ONLINE' Elevated Floating Button)"]
        EarningsCard["Today's Earnings Widget ('$248.50 · 12 Rides · 6h 15m Online')"]
        H3SurgeCanvas["Full-Screen Vector Map with H3 Surge Heatmap Layer (Res 7 Hexagons)"]
        QuestProgressBar["Active Shift Quest ('Complete 3 more rides for +$45 bonus' · [===--])"]
        DestinationFilter["Destination Filter Button ('Driving Toward: Mission District')"]
        DriverProfileAvatar["Driver Profile & Acceptance Rate Pill ('AR: 94% · CR: 1.2% · ★ 4.96')"]
    end
```
