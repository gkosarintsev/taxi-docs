...
```mermaid
graph TD
    subgraph SafetyToolkitModal [Safety Toolkit Bottom Sheet]
        Header["Safety Toolkit (Active Trip: #ride_9918)"]
        LiveTrustBadge["Verified Vehicle & Background Check Passed Badge"]
        PINModule["4-Digit Boarding PIN Verification Display (PIN: 4892)"]
        AudioRecordTile["In-Cabin Encrypted Audio Recording Tile (Toggle & Status)"]
        ShareTripTile["Share Live Trip with Trusted Contacts (SMS Link)"]
        EmergencyTile["Emergency Assistance Button (Call 911 / 112 with Live CAD Push)"]
        SafetyOperatorTile["24/7 Platform Safety Response Hotline"]
    end
```
