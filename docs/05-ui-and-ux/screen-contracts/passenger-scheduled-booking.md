...
```mermaid
graph TD
    subgraph ScheduledBookingView [Scheduled Ride Booking Viewport]
        DateTimeWheel["Date & Time Wheel Selector (Pickup Time: Aug 18, 2026 · 06:30 AM)"]
        PickupDropoffSummary["Address Summary Card (Pickup: Home · Dropoff: SFO Terminal 2)"]
        GuaranteedFareCard["Guaranteed Upfront Fare Card ('$42.00 Guaranteed Quote')"]
        ReservationFeeBadge["Reservation Badge ('Includes $3.50 Advance Reservation Fee')"]
        CancellationPolicyCard["Cancellation Policy Accordion ('Free cancellation up to 60 min before pickup')"]
        PrimaryConfirmCTA["'Schedule Ride for 06:30 AM' Primary Button"]
    end
```
